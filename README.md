...
- 🎭 **审查视角切换**：你是甲方还是乙方？一键设定审查立场，AI 将聪明地调整屁股，专为你挑出不利于己方的隐藏陷阱，争取更大利益。
...
**不支持直接运行。**经过核实，虽然现代版本的 WPS Office 推出了自己的 WPS 开放平台和 JavaScript API 规范，甚至号称其 API “高度兼容”微软的部分生态，但 WPS 并没有**直接原生支持**微软独家研发的 `Office.js Web Add-in` 插件框架系统。

本插件的底层架构深度绑定了微软 Office JS 容器的权限系统（如通过 `Office.context.document` 读取正文、操作批注等）。如果你是 WPS 深度用户，暂时无法直接挂载本插件，请更换到 **Microsoft Word** 以获得完整体验。
