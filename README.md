...
**支持（但依赖版本）。**本插件采用的是微软官方的 Office.js Web Add-in 框架架构。现代版本的 WPS Office（尤其是个人版/企业版的较新版本）已经开始兼容微软的这套 JavaScript API 规范，不再局限于老旧的 VBA 插件。

只要你的 WPS 版本足够新，像本作中使用的“获取选中文本”、“在光标处插入批注与高亮”等基础底层通讯接口，在 WPS 的 JS 容器内基本也是通用并能够正常运行的。当然，若追求绝对稳定的排版和视图体验，依然首推原生 Microsoft Word。
